...
export default function EditAccount() {
    ...
    useEffect(() => {
        ...
        fetch(config.apiUrl + '/finance/account/' + id, { credentials: 'include' })
            ...
            .catch(err => {
                ...
                notificationService.error("Error", "Could not load the account details.");
            })
            .finally(() => {
                setLoading(false);
            });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(config.apiUrl + '/finance/account/' + id, {
                credentials: 'include',
                method: "PUT",
                headers: {
            ...
            if (res.status === 200) {
                notificationService.success("Account Updated", "The account details have been updated successfully.");
                router.push('/dashboard/accounts/list');
            } else {
                const errData = await res.json();
                notificationService.error("Update Failed", errData.message || "Failed to update account.");
            }
        } catch (err) {
            console.error("Error updating account", err);
            notificationService.error("Error", "An unexpected error occurred while updating the account.");
        } finally {
            setIsSaving(false);
        }
    };
    ...
    return (
        <div className="space-y-6 max-w-2xl mx-auto pb-12">
            ...
            <div className="flex items-center justify-between border-b border-border pb-4">
                <div>
                    <h1 className="text-3xl font-extrabold text-text-primary tracking-tight">Edit Account</h1>
                    <p className="text-sm text-text-secondary mt-1">Modify accounting properties for this category.</p>
                </div>
                <button 
                    type="button"
            ...
            <form onSubmit={handleSubmit} className="bg-surface rounded-2xl border border-border p-6 shadow-sm space-y-6">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    >
                        ...
                        {allAccounts
                            // Exclude self from potential parents to avoid circular dependency
                            .filter(acco => acco.id !== id)
                            .map(acco => (
                ...
                <div className="flex justify-end gap-3 border-t border-border pt-6">
                    ...
                    >
                        {isSaving ? <ButtonLoader size={20} /> : "Update Account"}
                    </button>
                </div>
            </form>
        </div>
