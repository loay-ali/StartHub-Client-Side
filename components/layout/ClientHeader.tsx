        ...
        >
            <div className="flex items-center justify-between p-3 px-6 w-full">
                ...
                <nav className="flex items-center gap-3 md:gap-5">
                        ...
                        {MenuLinks.map(ele => {
                            ...
                            return (
                                <Link
                                    href={ele.href}
                                    key={ele.slug}
                                    className={`text-sm font-semibold relative py-2 px-4 transition-colors duration-300 rounded-xl whitespace-nowrap ${
                                        active 
                    ...
                    {/* Auth Area */}
                    <div className="hidden md:flex items-center gap-2 min-h-[38px]">
                        {isLoggedIn === null ? (
                            ...
                            <>
                                <Link href="/login" className="px-4 py-2 text-sm font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all duration-200 whitespace-nowrap flex items-center gap-1.5">
                                    <LogIn size={14} className="opacity-75" />
                                    <span>Login</span>
                                </Link>
                                <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
                                    <Link href="/register" className="px-4 py-2 text-sm font-bold bg-[#14b8a6] text-white hover:bg-[#0f766e] hover:shadow-[0_0_15px_rgba(20,184,166,0.3)] hover:shadow-md rounded-xl transition-all duration-200 text-center whitespace-nowrap flex items-center gap-1.5">
                                        <Sparkles size={13} />
                                        <span>Join Now</span>
                                    </Link>
                                </motion.div>
                    ...
                    >
                            ...
                            {MenuLinks.map(ele => {
                                ...
                                return (
                                    <Link
                                        ...
                                        className={`flex items-center justify-between text-sm font-semibold p-2.5 rounded-xl transition-all ${
                                            active
                        ...
                        <div className="flex flex-col gap-2">
                            {isLoggedIn === null ? (
                                ...
                                <>
                                    <Link onClick={() => setMobileMenuOpen(false)} href="/login" className="py-2.5 text-sm font-bold text-center text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all flex items-center justify-center gap-1.5 border border-slate-200 dark:border-slate-800">
                                        <LogIn size={14} />
                                        <span>Login</span>
                                    </Link>
                                    <Link onClick={() => setMobileMenuOpen(false)} href="/register" className="py-2.5 text-sm font-bold text-center bg-[#14b8a6] text-white hover:bg-[#0f766e] rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-sm">
                                        <Sparkles size={13} />
                                        <span>Join Now</span>
                                    </Link>
                                </>
