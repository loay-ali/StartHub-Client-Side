    ...
    <section className = 'grow-1 overflow-auto h-[0px]'>
        {data.messages.map(msg => {
            ...
            <div key = {msg._id} className = {'flex justify-'+ (msg.role == 'user' ? "start":"end") +' m-2'}>
                <article className = {(msg.role == 'user' ? "bg-white":"bg-primary-light") +' min-w-[165px] p-3 px-4 inline-block my-4 mx-2 rounded relative flex-col'}>
                    ...
                    <time className = 'text-white text-[12px] font-bold absolute top-[100%] inset-s-1 flex items-center gap-2'>
